...
r = HC.classify("cf 卡1 stop");
ok(r.kind === "regionEnd" && r.name === "卡1", "cf 触发区域结束");
const pReg = HC.processPage({
  name: "reg",
  code: [
    "it 标题 t 我的页面",
    "标题 in t",
...
const regHtml = HC.buildSinglePageHtml({ name: "reg" }, pReg && { name: "reg", code: "cf 详情\n    it 说明 t 详情内容\n    说明 in t\ncf 详情 stop", images: {}, files: {} });
...
const pApp = HC.processPage({ name: "a", code: "it 安装包 app\n安装包 in d", images: {}, files: { "安装包": fileObj } }, {});
ok(pApp.vars["安装包"].file && pApp.vars["安装包"].file.name === "HIC安装包.zip", "app 变量绑定文件元数据");
const appHtml = HC.buildSinglePageHtml({ name: "a" }, { name: "a", code: "it 安装包 app\n安装包 in d", images: {}, files: { "安装包": fileObj } });
ok(appHtml.indexOf("download=\"HIC安装包.zip\"") >= 0 && appHtml.indexOf("hic-dl") >= 0, "下载按钮渲染且带文件名");
// 未上传文件时的占位
const appHtmlEmpty = HC.buildSinglePageHtml({ name: "a" }, { name: "a", code: "it 安装包 app\n安装包 in d", images: {}, files: {} });
ok(appHtmlEmpty.indexOf("未上传文件") >= 0, "未上传文件时显示占位");
// usesDownload 检测
ok(HC.usesDownload({ pages: { x: { code: "it a app\na in d" } } }) === true, "usesDownload 识别 app+in d");
ok(HC.usesDownload({ pages: { x: { code: "it a t hi" } } }) === false, "usesDownload 未使用返回 false");
// 打包 .zip 时附带 app 文件
const appEntries = HC.buildZipEntries([{ id: "p1", name: "项目A", pages: { a: { name: "a", code: "it 安装包 app\n安装包 in d", images: {}, files: { "安装包": fileObj } } } }]);
ok(appEntries.some(function (e) { return e.data instanceof Uint8Array && e.data.length > 0; }), "zip 条目包含 app 二进制文件");

// ---- 19. v2.00 use 标准库 ----
r = HC.classify("use os, sys, math, json");
ok(r.kind === "use" && r.modules.length === 4, "use 解析多模块");
ok(Array.isArray(HC.STD_MODULES) && HC.STD_MODULES.indexOf("sqlite3") >= 0 && typeof HC.STDLIB.os === "string", "STDLIB 白名单导出");
const diag = HC.diagnose("use os\nuse notamodule");
