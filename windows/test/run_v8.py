...
with io.open(r"D:\simplelang-ide\js\hic.js", "r", encoding="utf-8") as f:
    hic_src = f.read()
...
var pr = SimpleLang.processPage(page, {});
...
var merged = SimpleLang.buildProjectMergedHtml(proj, {});
ok(merged.indexOf('data-page="我的项目::index"')>=0, "合并含index");
ok(merged.indexOf("简单上手的编程语言")>=0, "合并含文本");
ok(/<!DOCTYPE html>/i.test(merged), "HTML骨架");
var single = SimpleLang.buildSinglePageHtml({name:"我的项目"}, page);
ok(single.indexOf("简单上手")>=0 || single.indexOf("你好，Simple")>=0, "单页html");

// 8. ZIP
var entries = SimpleLang.buildZipEntries([proj]);
ok(entries.length===2, "zip两页");
var z = SimpleLang.zipFiles(entries.map(function(e){return {name:e.name, data:e.html};}));
